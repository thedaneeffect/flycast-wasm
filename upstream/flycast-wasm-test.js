...
async function runTest() {
  ...
  try {
    ...
    // 1. AUTO-SEND ntfy notification (plain text, human-readable)
    const ntfyTitle = `Flycast WASM — ${status}`;
    const ntfyLines = [];
    if (status === 'PASS') {
      ntfyLines.push(`BIOS rendered successfully.`);
      ntfyLines.push(`${visual.nonBlackRatio}% screen coverage, ${visual.uniqueColors} colors.`);
    } else if (status === 'FAIL_BLACK') {
      ntfyLines.push(`Black screen — BIOS did not render any graphics.`);
    } else if (status === 'FAIL_CRASH') {
      ntfyLines.push(`Emulator crashed during test.`);
      if (crashErrors.length > 0) ntfyLines.push(`Error: ${crashErrors[0].substring(0, 100)}`);
    } else if (status === 'FAIL_NO_VISUAL') {
      ntfyLines.push(`Some pixels but no meaningful visual output.`);
    } else if (status === 'FAIL_BEHAVIOR') {
      ntfyLines.push(`Emulation running but behavior is broken.`);
    } else {
      ntfyLines.push(`Test harness error.`);
    }
    ntfyLines.push(`${behavior.mainloopExits} emulation frames in ${TEST_DURATION_MS / 1000}s.`);
    ntfyLines.push(`ROM: ${ROM_CLICK_TEXT}`);
